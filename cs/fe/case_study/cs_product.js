...
function renderPage (productList) {
  let pageable = "";
   if (productList.number == productList.totalPages -1 && productList.number > 0) {
    pageable += `
...
function getProductInfo (id, name) {
  ...
  document.getElementById("deleteName").innerText = "Bạn có muốn xóa sản phẩm "+name+"?";
}
    ...
    elements += `<tr>
          ...
          <td>
              <button type="button"
                ...
                onclick="getProductInfo(${product.id}, '${product.name}')">
...
$("#add-product").submit(function (event) {
  ...
  let productTypeDTO = $("#productTypeDTO").val();
